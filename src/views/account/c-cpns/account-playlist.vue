...
const likePlaylist = ref<any>();

getAccountPlaylistAction(id).then(() => {
  const { playlist } = storeToRefs(accountPlaylistStore);
  const firstItem = playlist.value.shift();
  accountPlaylist.value = playlist.value;
  likePlaylist.value = firstItem;
  console.log("这是去除第一条数据后", likePlaylist.value.id);
  emit("like-playlist", likePlaylist.value.id);
});
const emit = defineEmits<{
  (e: "like-playlist", likePlaylist: number): void;
}>();
